...
export async function createUserAction(input: CreateUserInput) {
  try {
    ...
    // Use admin API to create user - this bypasses email confirmation requirement
    ...
    if (error) {
      console.error('[Server] User creation failed:', error);
    ...
    // Return only serializable data (userId string instead of full user object)
...
export async function createRestaurantAction(input: CreateRestaurantInput) {
  try {
    ...
    const restaurantData = {
      ...
      plan_tier: input.planId.includes('lite') ? 'lite' : input.planId.includes('starter') ? 'starter' : input.planId.includes('growth') ? 'growth' : 'pro',
