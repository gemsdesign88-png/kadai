...
interface AccountCreationProps {
  onComplete: (data: { email: string; password: string; fullName: string; phoneNumber: string; businessName: string }) => void;
  initialData: {
    email: string;
    password: string;
    fullName: string;
...
export default function AccountCreation({ onComplete, initialData }: AccountCreationProps) {
  ...
  const validate = () => {
    ...
    if (!formData.businessName) {
      newErrors.businessName = t.register.account.businessNameRequired;
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="mt-3 p-3 bg-gray-50 rounded-lg border border-gray-200">
              ...
              <ul className="space-y-1">
                <li className={`flex items-start gap-2 text-xs transition-colors ${
                  ...
                  <span>{language === 'id' ? 'Minimal 8 karakter' : language === 'zh' ? '至少8个字符' : 'At least 8 characters'}</span>
                </li>
                <li className={`flex items-start gap-2 text-xs transition-colors ${
                  ...
                  <span>{language === 'id' ? 'Kombinasi huruf besar & kecil' : language === 'zh' ? '大小写字母组合' : 'Mix of uppercase & lowercase'}</span>
                </li>
                <li className={`flex items-start gap-2 text-xs transition-colors ${
                  ...
                  <span>{language === 'id' ? 'Minimal 1 angka' : language === 'zh' ? '至少1个数字' : 'At least 1 number'}</span>
                </li>
                <li className={`flex items-start gap-2 text-xs transition-colors ${
                  ...
                  <span>{language === 'id' ? 'Minimal 1 karakter spesial (!@#$%^&*)' : language === 'zh' ? '至少1个特殊字符 (!@#$%^&*)' : 'At least 1 special character (!@#$%^&*)'}</span>
                </li>
              </ul>
            </div>
          </div>
          ...
          <div>
            <label htmlFor="businessName" className="block text-sm font-medium text-gray-700 mb-2">
              {t.register.account.businessName}
            </label>
            <input
              id="businessName"
              type="text"
              ...
              className={`w-full px-4 py-4 rounded-xl border ${
                errors.businessName ? 'border-red-300 focus:border-red-500 focus:ring-red-500' : 'border-gray-300 focus:border-[#FF5A5F] focus:ring-[#FF5A5F]'
              } focus:ring-2 focus:ring-opacity-50 transition-colors`}
              placeholder={t.register.account.businessNamePlaceholder}
            />
          ...
          <button
            ...
            className={`w-full py-4 font-bold rounded-full shadow-lg transition-all duration-200 transform ${
              isFormValid()
