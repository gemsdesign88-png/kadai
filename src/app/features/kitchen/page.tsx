      ...
      <section className="pt-32 pb-20 bg-gradient-to-b from-gray-50 to-white">
        <Container>
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            >
              <div className="absolute inset-0 bg-gradient-to-br from-orange-100 to-amber-100 rounded-3xl"></div>
              <div className="relative p-8">
                <div className="bg-white rounded-2xl p-6 shadow-2xl">
                  <h3 className="font-bold text-gray-900 mb-4">Kitchen Queue</h3>
                  {[
                    {table: "Meja 5", items: 3, time: "2m", urgent: true},
                    {table: "Meja 2", items: 2, time: "5m", urgent: false},
                    {table: "Meja 8", items: 4, time: "1m", urgent: true}
                  ].map((order, i) => (
                    <div key={i} className={`mb-3 p-4 rounded-xl border-2 ${
                      order.urgent ? 'bg-red-50 border-red-300' : 'bg-orange-50 border-orange-200'
                    }`}>
                      <div className="flex items-center justify-between mb-2">
                        <span className="font-bold text-gray-900">{order.table}</span>
                        <span className={`px-2 py-1 rounded-full text-xs font-bold ${
                          order.urgent ? 'bg-red-200 text-red-700' : 'bg-orange-200 text-orange-700'
                        }`}>
                          {order.time}
                        </span>
                      </div>
                      <div className="text-sm text-gray-600">{order.items} items</div>
                      <button className="mt-2 w-full py-2 bg-green-500 text-white rounded-lg text-sm font-bold">Complete</button>
                    </div>
                  ))}
                </div>
              </div>
