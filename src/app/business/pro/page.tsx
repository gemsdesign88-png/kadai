        ...
        if (data) {
          ...
          const filtered = data
            .filter((p: any) => {
              ...
              return id.startsWith('pro_starter') || id.startsWith('pro_growth') || id.startsWith('pro_pro') ||
                     p.plan_tier === 'professional' || p.plan_tier === 'service';
            })
                ...
                .map((plan: any) => {
                  ...
                  return (
                    <motion.div
                      ...
                      whileInView={{ opacity: 1, y: 0 }}
                      viewport={{ once: true }}
                      className={`bg-white rounded-3xl p-8 border transition-all relative overflow-hidden ${isPopular ? 'border-2 border-purple-600 shadow-2xl z-10' : 'border-gray-100 hover:shadow-xl'}`}
                    >
                      {isPopular && (
                        <div className="absolute top-0 left-0 right-0 bg-purple-600 text-white py-1.5 text-center text-xs font-bold tracking-wide">
                          {language === 'id' ? '⭐ PALING POPULER' : '⭐ MOST POPULAR'}
                        </div>
                      )}
                      <div className={`${isPopular ? 'mt-8' : ''} mb-8`}>
                        <div className="inline-block px-3 py-1 rounded-full text-xs font-bold text-white mb-3 bg-purple-600">
                          {tier}
                        </div>
                        <h3 className="text-2xl font-bold mb-2">{plan.name}</h3>
                        <div className="flex items-baseline gap-1 mb-4">
                          <span className="text-4xl font-black text-purple-600">{plan.price_display}</span>
                      ...
                      <Link href="/register">
                        <button className="w-full py-4 bg-purple-600 text-white rounded-xl font-bold hover:opacity-90 transition-opacity">
