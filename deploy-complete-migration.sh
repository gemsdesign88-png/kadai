...
# Complete VPS Migration Script for KadaiPOS
...
log_info "Step 1: Preparing local application..."

cd "$(dirname "$0")"

if [ ! -f "package.json" ]; then
    log_error "package.json not found. Are you in the kadaipos root directory?"
fi
...
# ===== STEP 3: SSH AND SETUP VPS =====
log_info "Step 3: Connecting to new VPS and setting up environment..."

ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 ${VPS_USER}@${NEW_VPS} 'bash -s' << 'ENDSSH'
...
log_info "Step 4: Transferring application to new VPS..."
...
log_info "Step 5: Extracting and installing application on VPS..."
...
log_info "Step 6: Configuring Nginx reverse proxy..."
...
log_info "Step 7: Setting up SSL certificate with Certbot..."
...
echo ""
echo "4. PM2 Logs (last 10 lines):"
pm2 logs --nostream kadaipos | tail -n 10
echo ""
...
echo "  IP: ${NEW_VPS}"
echo "  Hostname: ${NEW_VPS_HOSTNAME}"
echo ""
echo "Next Steps:"
echo "  1. Update DNS records to point to ${NEW_VPS}"
echo "  2. Monitor application at: https://kadaipos.id"
echo "  3. Check logs with: pm2 logs kadaipos"
